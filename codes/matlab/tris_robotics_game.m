...
% TRIS ROBOTICO INTERATTIVO - versione intelligente con Arduino
    ...
    view(0,90); xlabel('X'); ylabel('Y'); %vista dall'alto (piano xy)
    title('Tris Robotico Interattivo');
    robot.plot([0 0 0.8],'workspace',workspace,'delay',0.01); drawnow; % Plot condifurazione iniziale

    ik2R = @(x,y) safeIK2R(x,y,L1.a,L2.a);

    %% --- Video ---
    v = VideoWriter('tris_robotico_gioco.avi'); v.FrameRate = 25; open(v); % 25 fps per video fluido
...
function r = cellRow(c), r = 4 - ceil(c/3); end % Riga: 1→3, 2→3, 3→3, 4→2, ...
function c = cellCol(c), c = mod(c-1,3)+1; end % Colonna: ciclica 1,2,3
% Esempio:
% Cella 5 → Row=2, Col=2 (centro)
% Cella 1 → Row=3, Col=1 (angolo alto-sinistra)

function winner = checkWinner(b)
    winner = ' ';
    lines = [1 2 3; 4 5 6; 7 8 9; 1 4 7; 2 5 8; 3 6 9; 1 5 9; 3 5 7]; % righe; colonne; diagonali
...
function drawSymbol(robot, ik2R, v, s, pos, symbol)
    ...
    % Controllo del simbolo da disegnare
    if symbol=='X'
        th=linspace(0,1,40);
        % Prima diagonale
